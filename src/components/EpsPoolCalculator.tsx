...
/** ---------- DEFAULT PACKAGES (with ≥6 ppl sand filter) ---------- */
...
export default function EpsPoolCalculator() {
  ...
  // Warranty — now % of total client price
  const [warrantyPctOfClient, setWarrantyPctOfClient] = useState(1.5); // 1–2% typical
  ...
  const vesselCalcs = useMemo(() => {
    return vessels.map(v => {
      ...
      if (scopeInstallation) {
        install += clampN(epsWpPerSf) * finishSf;                            // EPS assembly + waterproofing labor
        if (useTileTurnkey) install += clampN(tileTurnkeyPerSf) * (floorSf + wallSf); // Tile turnkey on shell-only sf
        install += clampN(equipPlumbPerVessel);                               // MEP interconnects
        install += clampN(handrailInstallPerEa) * clampN(v.handrails);        // Handrails
  ...
  /** ---- project totals (with warranty % of client price) ---- */
  const project = useMemo(() => {
    ...
    // Rep onsite fee applies when installation scope is on
    const repFee = scopeInstallation ? clampN(repOnsiteFee) : 0;

    // Base hard costs BEFORE waste, OH&P, and warranty
    const hardCostsBase =
      materialsSubtotal +
    ...
    // Waste and OH&P are computed on base
    const wasteAmount = clampN(wastePct) / 100 * hardCostsBase;
    const ohpAmountPreWarranty = clampN(ohpPct) / 100 * hardCostsBase;

    // Warranty as % of total client price → solve for clientPrice:
    // clientPrice = (hardCostsBase + waste + ohp) / (1 - warrantyPct)
    const wPct = scopeWarranty ? clampN(warrantyPctOfClient) / 100 : 0;
    const numerator = hardCostsBase + wasteAmount + ohpAmountPreWarranty;
    const clientPrice = wPct < 1 ? numerator / (1 - wPct) : Infinity;

    const warrantyReserve = scopeWarranty ? clientPrice * wPct : 0;
    const ohpAmount = ohpAmountPreWarranty; // unchanged (based on base)
    const profit = ohpAmount;               // treat OH&P as the profit bucket
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <GridWrap>
        ...
        <Card title="OH&P + Waste + Warranty">
          ...
          <div style={{ fontSize: 12, color: "#6b7280", marginTop: 6 }}>
            Warranty is solved as a true % of client price (no circular math).
          </div>
        </Card>
      </GridWrap>
